class InMemoryFileSystem {
    ...
    cat(filePath) {
        const fullPath = this.joinPaths(this.currentPath, filePath);

        if (!this.fs[fullPath] || this.fs[fullPath].type !== 'file') {
            console.log(`Error: File '${filePath}' not found.`);
        } else {
            const content = this.fs[fullPath].content;
            console.log(content);
            return content;
        }
    }
